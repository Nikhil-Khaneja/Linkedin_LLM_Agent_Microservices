...
export default function Layout() {
  ...
  const photo   = localStorage.getItem(`photo_${user?.userId}`);

  useEffect(() => {
    // Static notifications - in real system these come from Kafka events
    setNotifications([
      { id:1, icon:'📋', text:'New applications received on your job postings', time:'Just now',    unread:true  },
      { id:2, icon:'💬', text:'You have unread messages in your inbox',         time:'5 min ago',   unread:true  },
      { id:3, icon:'🤝', text:'Someone sent you a connection request',           time:'30 min ago',  unread:false },
      { id:4, icon:'💼', text:'5 new jobs match your profile this week',        time:'2 hours ago', unread:false },
      { id:5, icon:'👀', text:'Your profile was viewed 12 times today',         time:'3 hours ago', unread:false },
    ]);
    ...
    searchTimer.current = setTimeout(async () => {
      try {
        const [jobsRes, membersRes] = await Promise.allSettled([
          axios.post(`${BASE.job}/jobs/search`,       { keyword:val, page_size:5 }),
          axios.post(`${BASE.member}/members/search`, { keyword:val, page_size:3 }),
        ]);
        const jobs    = jobsRes.status    === 'fulfilled' ? jobsRes.value.data.data?.jobs    || [] : [];
        ...
        <div style={S.inner}>
                  ...
                  {dropResults.jobs.map(j => (
                    <div key={j.job_id} style={S.dItem}
                      ...
                      <div>
                        ...
                        <p style={S.dSub}>{j.company_name} · {j.city||'Remote'}</p>
                      </div>
                    </div>
                  ))}
                </>}
                ...
                {dropResults.members.length > 0 && <>
                  ...
                  {dropResults.members.map(m => (
                    <div key={m.member_id} style={S.dItem}
                      onClick={() => { navigate('/connections'); setShowDrop(false); setSearch(''); }}>
                      <div style={{ ...S.dAvatar, background:'#057642' }}>{(m.first_name||'?')[0]}</div>
                      <div>
                        <p style={S.dTitle}>{m.first_name} {m.last_name}</p>
                        <p style={S.dSub}>{m.headline || 'LinkedIn Member'} {m.city ? `· ${m.city}` : ''}</p>
                      </div>
                    </div>
                  ))}
                </>}
          ...
          <nav style={S.nav}>
            ...
            <div ref={notifRef} style={{ position:'relative', display:'flex' }}>
              <button onClick={() => { setShowNotif(!showNotif); setUnread(0); }}
              ...
              {showNotif && (
                <div style={S.notifBox}>
                  ...
                  {notifications.map(n => (
                    <div key={n.id} style={{ ...S.notifItem, background: n.unread ? '#f0f7ff' : '#fff' }}
